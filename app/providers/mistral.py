...
class MistralProvider(BaseProvider):
    ...
    def __init__(self, base_url: str = "https://api.mistral.ai/v1", model_mapper: ModelMapper = None):
    ...
    async def chat_completion(
        ...
        """Execute chat completion via Mistral API."""
        ...
        try:
            ...
            return ChatCompletionResponse(
                id=data.get("id", f"mistral-{int(time.time())}"),
            ...
            if 500 <= status_code < 600:
                raise ProviderUnavailableError(
                    provider=self.name,
                    status_code=status_code,
                    message=f"Mistral server error: {response_text[:200]}",
                )
            ...
            await logger.aerror("Mistral request timeout", error=str(e))
            raise ProviderUnavailableError(
                provider=self.name,
                status_code=504,
            ...
            await logger.aerror("Mistral request failed", error=str(e))
            raise