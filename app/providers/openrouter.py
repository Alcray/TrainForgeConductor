...
class OpenRouterProvider(BaseProvider):
    """OpenRouter provider — access to many free models."""

    name = "openrouter"

    def __init__(self, base_url: str = "https://openrouter.ai/api/v1", model_mapper: ModelMapper = None):
    ...
    async def chat_completion(
        ...
        headers = {
            ...
            "HTTP-Referer": "https://github.com/Alcray/TrainForgeConductor",
            "X-Title": "TrainForgeConductor",
        }
        ...
        try:
            ...
            if 500 <= status_code < 600:
                raise ProviderUnavailableError(
                    provider=self.name,
                    status_code=status_code,
                    message=f"OpenRouter server error: {response_text[:200]}",
                )
            ...
            await logger.aerror("OpenRouter request timeout", error=str(e))
            raise ProviderUnavailableError(
                provider=self.name,
                status_code=504,
            ...
            await logger.aerror("OpenRouter request failed", error=str(e))
            raise