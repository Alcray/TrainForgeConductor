...
class GroqProvider(BaseProvider):
    ...
    def __init__(self, base_url: str = "https://api.groq.com/openai/v1", model_mapper: ModelMapper = None):
    ...
    async def chat_completion(
        ...
        """Execute chat completion via Groq API."""
        ...
        try:
            ...
            if 500 <= status_code < 600:
                raise ProviderUnavailableError(
                    provider=self.name,
                    status_code=status_code,
                    message=f"Groq server error: {response_text[:200]}",
                )
            ...
            await logger.aerror("Groq request timeout", error=str(e))
            raise ProviderUnavailableError(
                provider=self.name,
                status_code=504,
            ...
            await logger.aerror("Groq request failed", error=str(e))
            raise