...
class SambaNovaProvider(BaseProvider):
    ...
    def __init__(self, base_url: str = "https://api.sambanova.ai/v1", model_mapper: ModelMapper = None):
    ...
    async def chat_completion(
        ...
        """Execute chat completion via SambaNova API."""
        ...
        try:
            ...
            return ChatCompletionResponse(
                id=data.get("id", f"sambanova-{int(time.time())}"),
            ...
            if 500 <= status_code < 600:
                raise ProviderUnavailableError(
                    provider=self.name,
                    status_code=status_code,
                    message=f"SambaNova server error: {response_text[:200]}",
                )
            ...
            await logger.aerror("SambaNova request timeout", error=str(e))
            raise ProviderUnavailableError(
                provider=self.name,
                status_code=504,
            ...
            await logger.aerror("SambaNova request failed", error=str(e))
            raise